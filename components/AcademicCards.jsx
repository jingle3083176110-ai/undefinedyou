    ...
    <div className="grid gap-px border border-ink/20 bg-ink/20 md:grid-cols-2">
      {cards.map((card) => (
        <Link key={card.href} href={card.href} className="group min-h-72 bg-cream p-7 transition-colors hover:bg-[#f0ece3]">
          <p className="font-mono text-xs uppercase tracking-[.2em] text-muted">{card.number} / {card.eyebrow}</p>
          ...
          <span className="mt-8 inline-block border-b border-ink/40 pb-1 font-mono text-xs uppercase tracking-[.2em] text-ink">{t("academic.open")} ↗</span>
        </Link>
      ))}
    </div>
  );
