  ...
  return (
    <main className="min-h-screen bg-[#1c1a18] px-6 pb-28 pt-32 text-[#f5f0e8] md:px-16 lg:px-24">
      <div className="mx-auto max-w-6xl">
        <Link href="/journal" className="font-mono text-xs uppercase tracking-[0.24em] text-[#c9c0b5] transition-colors hover:text-white">← Journal</Link>

        <header className="mt-14 border-b border-white/20 pb-10">
          <p className="font-mono text-xs uppercase tracking-[0.28em] text-[#c9c0b5]">02 / A personal shelf</p>
          <h1 className="mt-6 font-display text-[clamp(3.5rem,8vw,7rem)] leading-[1.02] tracking-normal">Reading Room</h1>
          <p className="mt-8 max-w-xl font-sans text-lg leading-relaxed text-[#c9c0b5]">Books kept close, pages underlined, and notes written after the reading slows down.</p>
        </header>

        <section className="py-14 md:py-20">
          <div className="mb-8 flex items-end justify-between gap-6">
            <div>
              <p className="font-mono text-xs uppercase tracking-[0.24em] text-[#c9c0b5]">On the shelf / {shelfCaption}</p>
              <p className="mt-3 text-sm text-[#a9a095]">Swipe sideways to browse the shelf</p>
            </div>
          ...
          <div ref={shelfRef} onScroll={updateActiveBook} className="-mx-6 flex snap-x snap-mandatory gap-5 overflow-x-auto pb-8 pt-5 [scrollbar-color:#776f66_transparent] [scrollbar-width:thin] md:-mx-16 lg:-mx-24" style={{ paddingLeft: "calc(50% - clamp(130px, 18vw, 150px))", paddingRight: "calc(50% - clamp(130px, 18vw, 150px))" }}>
            {books.map((item, index) => {
              ...
              return <article data-book-index={index} key={item.slug} className="relative flex h-[390px] w-[260px] shrink-0 snap-center flex-col justify-between overflow-hidden p-7 md:h-[450px] md:w-[300px] md:p-8" style={{ backgroundColor: item.color, opacity, filter: `blur(${blur}px) saturate(${1 - Math.min(peripheralDistance * 0.08, 0.15)})`, transition: "opacity 90ms linear, filter 90ms linear", boxShadow: distance < 0.15 ? "0 30px 55px rgba(0,0,0,.42)" : "0 15px 32px rgba(0,0,0,.18)" }}>
                ...
                <div className="relative"><p className="font-mono text-[10px] uppercase tracking-[0.24em] text-white/70">{String(index + 1).padStart(2, "0")} / Reading notes</p><p className="mt-4 font-sans text-sm text-white/75">{item.author}</p></div>
                <div className="relative"><h2 className="font-display text-5xl leading-[1.02] tracking-normal text-white">{item.title}</h2><p className="mt-5 font-mono text-[10px] tracking-[0.18em] text-white/70">RATING {item.rating}/10</p></div>
              </article>;
            })}
          </div>
        </section>
        ...
        <section className="border-t border-white/20 pt-12">
          <div className="max-w-3xl"><p className="font-mono text-xs uppercase tracking-[0.28em] text-[#c9c0b5]">Notes / {book.title}</p><h2 className="mt-5 font-display text-5xl leading-none md:text-6xl">{book.title}</h2><p className="mt-4 font-sans text-lg text-[#c9c0b5]">{book.author}</p></div>
