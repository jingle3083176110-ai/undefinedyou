...
test("gallery is newest first and each photo belongs to a collection", () => {
  ...
  assert.ok(galleryPhotos.some((photo) => photo.collectionType === "workshops"));
...
test("initial theme and workshop reference supplied photographs", () => {
  ...
  assert.equal(photoWorkshops.length, 3);
  assert.ok(photoWorkshops.every((workshop) => workshop.coverId && workshop.imageIds.length > 0));
});
